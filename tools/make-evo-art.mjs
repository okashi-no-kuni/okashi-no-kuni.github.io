...
/* ---------- c_hinotama（ひのたま）——**頭の 炎冠が 太く 高く 育つ** ----------
   Batch 2 の 7つめ。`c_firebird` が **横・ななめ下**へ 開く 翼なので、
   こちらは **上だけ**へ 育ちます。

     c_onibi    … 離れた **3つの 独立した 炎** → こちらは **独立炎を 1つも 置かない**
     c_firebird … **下外へ 開く 二重翼** → こちらは よこに 太らせない
     c_ghost    … 下へ 広がる ヴェール
     c_lavagolem… 浮遊岩

   base の 実測 ——bbox x47..208 / y25..230（162x206）。上の あきは **25px**
   しか なく、しかも 炎の 真上（x92..170）は **15〜36px**。ゆとりは
   **左右の 肩**（x≤84・x≥172 で 53〜133px）に あります。
   だから 舌は **肩から 立てて 内へ よせ**、てっぺんを かせぎます。

   **body shift は 0 が 最適**です。上へ S px ずらすと 追加炎が つかえる
   上の あきも 同じだけ 減るので、総丈は 221-S に なって **ずらすほど
   低く なります**（設計フェーズで 実測）。

   本体の 冠の 輪郭（分離炎を のぞいて 実測）——
     x80..84 で y68〜70（左の 山）／x96 で y78（谷）／
     x140..148 で y37（まん中の 山）／x152 で y72（谷）／
     x164..172 で y63〜69（右の 山）
   新しい 舌は **その 3つの 山の 上**に 立てます。そうすると
   「新しい 炎を 3本 足した」では なく「もとの 舌が 育った」に 見えます */

/* 炎冠。**1本ずつ 別の パスに しない**（`crestPath` と 同じ 話）。
   舌を たてに サンプリングして **x ごとの 最小 y**を とり、1つの パスに します。
   `lean` / `curve` で S字に そらせるので、まっすぐな 三角（角・トゲ・王冠）に
   なりません */
/* 炎冠。**1つの パス**で 3つの 舌を つなげて えがきます。
   パラメトリックな union（`crestPath` の やりかた）も 試しましたが、
   たいらな 頂点と 角ばった くぼみに なり、**王冠・リボン・耳**に
   見えました（4回 やりました）。base の 舌と 同じ
   「ふくらんで → くびれて → 先が そる」S字は、ベジエで 直に 書くのが 確実です。

   すべての 数は base の 実測から ——冠の 山は x82(y68) / x144(y37) / x166(y65)、
   谷は x96(y78) / x152(y72)。舌は **その 山の 上**に 立てるので
   「新しい 炎を 3本 足した」では なく「もとの 舌が 育った」に 見えます。
   くぼみ（108,46）（152,40）は **本体の 中**なので かくれ、
   見えるのは 冠から 立ちあがる ところ だけ です */
const HT_CROWN = 'M84,104'
  + ' C70,88 66,66 76,50'        // 左の 舌：低い ところで **外へ ふくらむ**
  + ' C82,38 86,28 96,16'        //          くびれてから 先が 内へ そる（S字）
  + ' C98,30 102,40 108,48'      // くぼみ①（**本体の 中**なので かくれる）
  + ' C114,40 116,28 124,18'     // まん中の 舌（いちばん 高い）
  + ' C128,14 131,12 135,13'     //          先は すこし まるく
  + ' C139,24 144,34 152,42'     // くぼみ②（本体の 中）
  + ' C158,34 163,26 170,18'     // 右の 舌
  + ' C174,15 178,15 181,19'     //          先は すこし まるく
  + ' C186,42 187,74 178,104'    // 右の へりを 本体へ もどす
  + ' C148,112 112,112 84,104 Z';// 下は 本体の 中（かくれる）

const hinotamaSvg = () => `<svg xmlns="http://www.w3.org/2000/svg" width="${N}" height="${N}">
<defs>
 <linearGradient id="htF" x1="0" y1="0" x2=".2" y2="1">
  <stop offset="0" stop-color="#fdb98e"/><stop offset=".5" stop-color="#fe9f83"/>
  <stop offset="1" stop-color="#f4a081"/></linearGradient>
</defs>
<path d="${HT_CROWN}" fill="url(#htF)" stroke="#cd7060"
      stroke-width="2.6" stroke-linejoin="round"/>
</svg>`;
...
/* ---------- c_mummy（ミイラ）——**ほどけた 包帯が 不規則に 垂れて、
   外形そのものが「ふさ」に かわる** ----------
   Batch 3 の 2体目。`c_octopus_e1` が「太い 曲線が 育つ」なら、
   こちらは **輪郭の 連続性を こわす**のが 主役です。

   base の 実測 ——bbox x33..222 / y26..230（190x205）・fill .615・
   左右対称度 IoU **0.926**（右が 2.3% 多い ＝ **すでに ゆるく 非対称**）。
   ほどけ端は すでに 5か所 ——①頭の右 x178..214/y78..102（**左には ない**）／
   ②左うで x56..80/y155..190（y182 で 2本に 割れる）／③右うで x179..198／
   ④左のすそ x33..84/y208..228／⑤右のすそ x170..222。

   **ところが 小サイズでは その 5つとも 輪郭に 吸収されて います。**
   行ごとの run を 数えると、外形が 破れて いる 行は
   **36px で 2/30・24px で 1/20** しか ありません。だから この子の
   進化差は 面積でも 幅でも なく、**破れた 行を ふやす こと**です。

   実測で 分かった こと（設計フェーズ）——

     C1 下へ 垂らす   … 36px 6/33。下の あきは B 1,227px²・足もと 16px しか
                        なく わく外へ 出る。しかも `c_ghost_e1` の すそに 近づく
     C2 左右へ 広げる … 面積を +20.7% ふやしても **破れは 2/30 の まま**。
                        横へ 出した 帯は 本体と 同じ 行に つくので
                        **輪郭を「広げる」だけで「破らない」**
     C3 不規則        … **36px 7/31・24px 4〜6/21**。採用

   **「大きくすれば 進化差が 出る」が 成りたたない ゆいいつの 例**です */

/* 帯の 太さと 本数は 実測で 決めました（設計フェーズ）。
     太さ w=10〜14 … 36px は 破れるが **24px で 4〜5/21 に 落ちる**
     太さ **w=18〜20** … 24px で **6/21**。いちばん よい
     太さ w≥24     … 帯どうし・帯と 本体が 融合して **かえって 減る**
     本数 **5〜7本** … **6本は 5本より 24px で 悪く なりました**。
                      近づけすぎると すき間が うまって 破れが 消えます

   付け根は **run が 1つの 行**に 取ります（`c_octopus_e1` の 教訓）。
   y94..102 / y126 / y162..190 / y210..230 は run が 割れて いるので、
   そこに つなぐと 部位の すき間から 宙に 浮いた 継ぎはぎが 見えます */
const MWRAP = [
  /* 右：頭の ほどけ端（base に すでに ある ①）を 上外へ 出して **垂らす**。
     base の 非対称を **そのまま 増幅**する ——左には この 帯が ない */
  { P: [[192,86],[210,75],[225,76],[232,91],[227,106]],  w: t => 21*(1-0.60*Math.pow(t,0.9))*(1+0.13*Math.sin(3*Math.PI*t)) },
  /* 右：うでから 外へ 出て 垂れる */
  { P: [[188,148],[210,153],[226,167],[227,187],[218,199]], w: t => 20*(1-0.58*Math.pow(t,0.9))*(1+0.12*Math.sin(3*Math.PI*t)) },
  /* 右：すそから 下外へ */
  { P: [[166,200],[186,209],[200,221],[202,233]],       w: t => 19*(1-0.55*Math.pow(t,0.9))*(1+0.11*Math.sin(3*Math.PI*t)) },
  /* 左：頭よこから 上外へ。**右より みじかく**（鏡像に しない）*/
  { P: [[80,110],[57,101],[37,102],[29,116],[33,130]],   w: t => 19*(1-0.58*Math.pow(t,0.9))*(1+0.12*Math.sin(3*Math.PI*t)) },
  /* 左：うでから 横外へ 出て、右より **ふかく** 垂れる */
  { P: [[72,150],[48,158],[30,174],[26,194],[33,208]],   w: t => 21*(1-0.60*Math.pow(t,0.9))*(1+0.13*Math.sin(3*Math.PI*t)) },
  /* 左：すそから 下外へ */
  { P: [[88,202],[69,212],[56,225],[54,234]],            w: t => 19*(1-0.55*Math.pow(t,0.9))*(1+0.11*Math.sin(3*Math.PI*t)) },
];

const mummySvg = () => `<svg xmlns="http://www.w3.org/2000/svg" width="${N}" height="${N}">
<defs>
 <linearGradient id="mwF" x1=".2" y1="0" x2=".8" y2="1">
  <stop offset="0" stop-color="#f7f6fb"/><stop offset=".55" stop-color="#e6e3f0"/>
  <stop offset="1" stop-color="#d1cbe1"/></linearGradient>
</defs>
${MWRAP.map(a => `<path d="${flowPath(catmull(a.P, 16), a.w)}" fill="url(#mwF)"`
...
export const PLAN = {
  ...
  c_hinotama: {
    kind:  'deco',
    base:  'art/sprites/hinotama.png',        // ひのたま（**読むだけ**）
  ...
  c_mummy: {
    kind:  'deco',
    base:  'art/sprites/mummy.png',           // ミイラ（**読むだけ**）
    out:   'art/sprites/c_mummy_e1.png',
