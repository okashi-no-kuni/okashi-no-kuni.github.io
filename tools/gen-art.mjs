...
const ART = {
  ...
  マカロンタワー: { key:'tower', p:'A pyramid of macarons in exactly four rows, counted from the top: row 1 is a single macaron, row 2 has exactly two macarons, row 3 has exactly three, row 4 at the bottom has exactly four. Exactly ten macarons in total. Do not add any further row and do not repeat a row count. All the same size. Four colours only: sugar plum pink #ED70B1, sky blue #8FD8F0, mint green #8FE3C4 and butter yellow #FFE08A. Every colour is a soft milky pastel tint of those hues, light and gentle, never neon and never deep. The pink must stay clearly rosy pink like #ED70B1, not mauve, not purple and not orange. The face is on the single pink macaron at the top. Each is a proper macaron with two round shells and a filling.' },
