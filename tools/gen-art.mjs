...
const ART = {
  ...
  マカロンタワー: { key:'tower', p:'A pyramid of macarons in exactly four rows, counted from the top: row 1 is a single macaron, row 2 has exactly two macarons, row 3 has exactly three, row 4 at the bottom has exactly four. Exactly ten macarons in total. Do not add any further row and do not repeat a row count. All the same size. The colours are sugar plum, pale sky blue, mint and soft butter yellow. Sugar plum is a gentle pastel plum pink that leans towards purple, not a warm pink and never orange or salmon; keep it pale. The face is on the single sugar plum macaron at the top. Each is a proper macaron with two round shells and a filling.' },
